...
namespace Streamish.Tests
{
    public class VideoControllerTests
    {
        ...
        [Fact]
        public void Search_By_Title_Returns_Video_With_Given_Title()
        {
            //Arrange
            var video = 1;
            var videos = CreateTestVideos(5);
            videos[0].Title = "Eragon";
            ...
            //Act 
            var result = controller.Search("Eragon", true);

            //Assert 
            var okResult = Assert.IsType<OkObjectResult>(result);
            var actualVideo = Assert.IsType<List<Video>>(okResult.Value);
            Assert.Equal(video, actualVideo.Count);
        }
